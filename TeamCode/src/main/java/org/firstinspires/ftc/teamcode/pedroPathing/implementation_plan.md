...
## 1. PHÂN TÍCH CHU TRÌNH TUẦN HOÀN 1 BOX CHUẨN THỰC TẾ
...
graph TD
    ...
    B --> C["Step 1: READY (S1 hạ hàng gắp, S4 quay trước, S5 quay sang cột)"]
    C --> D["Step 2: EXTEND (CHỈ S2 telescope vươn ra gắp)"]
    D --> E["Step 3: GRAB (Gripper S3 kẹp chặt)"]
    E --> F["Step 4: LIFT UP (S1 nhấc nhẹ tránh vướng kệ)"]
    F --> G["Step 5: RETRACT (S2 telescope co về)"]
    G --> H["Step 6: READY STORE (S1 cực cao, S4 ngửa sau, S5 sang khay cất)"]
    H --> I["Step 7: STORE (S1 hạ khay robot, S2 vươn nhẹ, nhả S3)"]
...
| **1** | **READY** | `S1_ROW1/2` | `S2_HOME` (0.75) | `S3_OPEN` (0.0) | `S4_GRAB` (0.0) | `S5_LEFT/RIGHT` | S1 hạ hàng gắp, swing S5 sang cột, S4 hướng trước |
...
| **6** | **H-STORE**| `S1_HIGH_STORE` (0.2)| giữ Step 5 | giữ Step 5 | `S4_STORE` (0.75)| `S5_STORE1-4` | S1 dâng cực cao, S4 ngửa hẳn ra sau, S5 sang khay cất |
| **7** | **STORE** | `S1_STORE` (0.0) | `S2_STORE` (0.75)| `S3_OPEN` (0.0) | giữ Step 6 | giữ Step 6 | Hạ cánh tay, S2 duỗi nhẹ, mở kẹp S3 thả hộp |
...
- **MODE 1 - 8: Tune theo bước của chu trình thực tế:**
  * Robot tự động di chuyển đến tư thế của bước cơ học đó.
  * Bạn sử dụng **Left Stick Y** để jog chỉnh góc của servo chủ chốt trong bước đó (ví dụ Step 1 chỉnh cột S5 hoặc giữ LT chỉnh hàng S1, Step 2 jog S2 vươn duỗi).
