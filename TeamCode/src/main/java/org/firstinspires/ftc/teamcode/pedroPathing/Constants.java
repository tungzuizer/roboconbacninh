...
public class Constants {
    public static FollowerConstants followerConstants = new FollowerConstants()
            .mass(5)
            .forwardZeroPowerAcceleration(-108.29166758285331)
            .lateralZeroPowerAcceleration(-104.3634241994715)

            // ===== Translational PIDF (vị trí x-y) =====
            // PIDF chính: dùng khi robot còn CÁCH3 XA target hơn translationalPIDFSwitch (inch)
            .translationalPIDFCoefficients(new PIDFCoefficients(0.06, 0, 0.0001, 0.033))
            ...
            .secondaryTranslationalPIDFCoefficients(new PIDFCoefficients(0.2, 0, 0.001, 0.03))

            // ===== Heading PIDF (góc xoay) =====
            .headingPIDFCoefficients(new PIDFCoefficients(0.9, 0, 0.0001, 0.05))
            ...
            .secondaryHeadingPIDFCoefficients(new PIDFCoefficients(4, 0, 0.05, 0.0008))

            // ===== Drive PIDF (lực kéo bánh xe theo path) =====
            // PIDF chính: dùng khi vận tốc còn cao / xa target
            .drivePIDFCoefficients(new FilteredPIDFCoefficients(0.005, 0, 0.00001, 0.6, 0.008))
    ...
    public static MecanumConstants driveConstants = new MecanumConstants()
            ...
            .xVelocity(40.121864859513416)   // <-- cần đo lại với maxPower(0.5)
            .yVelocity(32.53757098340613)    // <-- cần đo lại với maxPower(0.5)
            ;


    public static PinpointConstants localizerConstants = new PinpointConstants()
            .forwardPodY(5.90551181)
            .strafePodX(-0.62992126)
     ...
     *
     * - brakingStrength: số càng lớn thì phanh càng mạnh/gắt. Vì robot bạn đã chạy chậm
     *   (maxPower 0.5) nên không cần phanh quá mạnh, để 1.0 - 1.25 là hợp lý.
     * - brakingStart: tỉ lệ (0-1) trên path tại đó bắt đầu giảm tốc. Để 0.85 nghĩa là
     *   robot bắt đầu giảm tốc khi đi được 85% quãng đường, giúp dừng êm hơn ở cuối.
     */
